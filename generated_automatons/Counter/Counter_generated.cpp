...
const std::string AUTOMATON_NAME = "Counter";
...
// Action function for state: Counting
void action_COUNTING() {
    std::cout << "[ACTION] Executing action for state Counting" << std::endl;
    // User-defined action code:
    counter = counter + 1; output("count_out", counter); output("status_out", "Counting");
}

// Action function for state: Idle
void action_IDLE() {
    std::cout << "[ACTION] Executing action for state Idle" << std::endl;
    // User-defined action code:
    counter = 0; output("count_out", counter); output("status_out", "Reset");
}

// Action function for state: Limit_Reached
void action_LIMIT_REACHED() {
    std::cout << "[ACTION] Executing action for state Limit_Reached" << std::endl;
    // User-defined action code:
    output("status_out", "Limit Reached");
}
...
// Function names include a unique index generated during JSON serialization.
    ...
    // Guard function for transition #0 (Source: Idle, Target: Counting)
    bool check_guard_0() {
    try {
    ...
    // Guard function for transition #1 (Source: Counting, Target: Counting)
    bool check_guard_1() {
    try {
         // User-defined guard condition code:
         // Uses original variable names and calls valueof("input_name") for inputs.
    ...
    // Guard function for transition #2 (Source: Counting, Target: Limit_Reached)
    bool check_guard_2() {
    try {
         // User-defined guard condition code:
         // Uses original variable names and calls valueof("input_name") for inputs.
...
void executeCurrentStateAction() {
    ...
    switch (currentState) {
        ...
        case State::STATE_COUNTING: // Case for state: Counting
            action_COUNTING();      // Call generated action function
            break;
        
        case State::STATE_IDLE: // Case for state: Idle
            action_IDLE();      // Call generated action function
            break;
        
        case State::STATE_LIMIT_REACHED: // Case for state: Limit_Reached
            action_LIMIT_REACHED();      // Call generated action function
            break;
    ...
    { // Scope for temporary stringstream
        std::stringstream ss;
        ss << counter; // Convert variable value to string
        engine.sendVarUpdate("counter", ss.str()); // Send update
    } 
    
    { // Scope for temporary stringstream
        std::stringstream ss;
        ss << limit; // Convert variable value to string
        engine.sendVarUpdate("limit", ss.str()); // Send update
    } 
    ...
    do {
        ...
        if (!event) {
            switch(currentState) {
                ...
                // Check transitions originating from state: Counting
                case State::STATE_COUNTING: {
                    bool guard_ok; // Variable to store guard evaluation 
                     ...
                     end_switch_immediate_STATE_COUNTING:;
                    break;
                    }
                
                // Check transitions originating from state: Idle
                case State::STATE_IDLE: {
                    bool guard_ok; // Variable to store guard evaluation 
                     ...
                     end_switch_immediate_STATE_IDLE:;
                    break;
                    }
                
                // Check transitions originating from state: Limit_Reached
                case State::STATE_LIMIT_REACHED: {
                    bool guard_ok; // Variable to store guard evaluation 
                     ...
                     end_switch_immediate_STATE_LIMIT_REACHED:;
                    break;
                    }
        ...
        // This phase runs only if an external event *is* being processed and no immediate transition was taken in Phase 1.
            ...
            switch(currentState){
                 ...
                 // Check transitions from state: Counting
                 case State::STATE_COUNTING:{
                    bool guard_ok;
                     
                            // Consider transition #0: Idle -> Counting
                            
                     
                            // Consider transition #1: Counting -> Counting
                                ...
                                if(guard_ok) {
                                    // Check if immediate or delayed.
                                     // Immediate event transition.
                                        next_state_event_candidate = State::STATE_COUNTING; // Set target state.
                                        event_transition_found = true; // Mark event transition found.
                                        goto end_switch_event_STATE_COUNTING; // Use goto to exit the inner loop and switch for this state.
                                    
                                }
                             }
                        
                     
                            // Consider transition #2: Counting -> Limit_Reached
                             ...
                             if (eventName == "increment") {
                                ...
                                if(guard_ok) {
                                    // Check if immediate or delayed.
                                     // Immediate event transition.
                                        next_state_event_candidate = State::STATE_LIMIT_REACHED; // Set target state.
                                        event_transition_found = true; // Mark event transition found.
                                        goto end_switch_event_STATE_COUNTING; // Use goto to exit the inner loop and switch for this state.
                            ...
                            // Consider transition #3: Counting -> Idle
                             ...
                             if (eventName == "reset") {
                                ...
                                if(guard_ok) {
                                    // Check if immediate or delayed.
                                     // Immediate event transition.
                                        next_state_event_candidate = State::STATE_IDLE; // Set target state.
                                        event_transition_found = true; // Mark event transition found.
                                        goto end_switch_event_STATE_COUNTING; // Use goto to exit the inner loop and switch for this state.
                            ...
                            // Consider transition #4: Limit_Reached -> Idle
                            
                     
                     end_switch_event_STATE_COUNTING:;
                    break;
                 }
                 
                 // Check transitions from state: Idle
                 case State::STATE_IDLE:{
                    bool guard_ok;
                                ...
                                if(guard_ok) {
                                    // Check if immediate or delayed.
                                     // Immediate event transition.
                                        next_state_event_candidate = State::STATE_COUNTING; // Set target state.
                                        event_transition_found = true; // Mark event transition found.
                                        goto end_switch_event_STATE_IDLE; // Use goto to exit the inner loop and switch for this state.
                     ...
                     end_switch_event_STATE_IDLE:;
                    break;
                 }
                 
                 // Check transitions from state: Limit_Reached
                 case State::STATE_LIMIT_REACHED:{
                    bool guard_ok;
                                ...
                                if(guard_ok) {
                                    // Check if immediate or delayed.
                                     // Immediate event transition.
                                        next_state_event_candidate = State::STATE_IDLE; // Set target state.
                                        event_transition_found = true; // Mark event transition found.
                                        goto end_switch_event_STATE_LIMIT_REACHED; // Use goto to exit the inner loop and switch for this state.
                                    
                                }
                             }
                        
                     
                     end_switch_event_STATE_LIMIT_REACHED:;
                    break;
                 }
...
void handleStatusRequestCallback() {
    ...
    
    {
        std::stringstream ss;
        ss << counter;
        engine.sendVarUpdate("counter", ss.str());
    }
    
    {
        std::stringstream ss;
        ss << limit;
        engine.sendVarUpdate("limit", ss.str());
    }
...
int main(int argc, char *argv[]) {
    ...
    stateNameToEnum["Counting"] = State::STATE_COUNTING; // Map name to enum
    stateEnumToName[State::STATE_COUNTING] = "Counting"; // Map enum to name
    
    stateNameToEnum["Idle"] = State::STATE_IDLE; // Map name to enum
    stateEnumToName[State::STATE_IDLE] = "Idle"; // Map enum to name
    
    stateNameToEnum["Limit_Reached"] = State::STATE_LIMIT_REACHED; // Map name to enum
    stateEnumToName[State::STATE_LIMIT_REACHED] = "Limit_Reached"; // Map enum to name
    ...
    if (currentState == State::STATE_NULL && "Idle" != "") {
         std::cerr << "[ERROR] Initial state 'Idle' issue!" << std::endl; return 1;
    } else if ("Idle" == "") {
