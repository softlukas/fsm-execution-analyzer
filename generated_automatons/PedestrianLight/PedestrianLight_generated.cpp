...
const std::string AUTOMATON_NAME = "PedestrianLight";
...
// Action function for state: Cars_Green
void action_CARS_GREEN() {
    std::cout << "[ACTION] Executing action for state Cars_Green" << std::endl;
    // User-defined action code:
    output("car_light", "GREEN"); output("ped_light", "RED");
}

// Action function for state: Cars_Yellow
void action_CARS_YELLOW() {
    std::cout << "[ACTION] Executing action for state Cars_Yellow" << std::endl;
    // User-defined action code:
    output("car_light", "YELLOW");
}

// Action function for state: Peds_Flash_Off
void action_PEDS_FLASH_OFF() {
    std::cout << "[ACTION] Executing action for state Peds_Flash_Off" << std::endl;
    // User-defined action code:
    flash_count++; output("ped_light", "BLACK");
}

// Action function for state: Peds_Flash_On
void action_PEDS_FLASH_ON() {
    std::cout << "[ACTION] Executing action for state Peds_Flash_On" << std::endl;
    // User-defined action code:
    flash_count++; output("ped_light", "GREEN");
}

// Action function for state: Peds_Green
void action_PEDS_GREEN() {
    std::cout << "[ACTION] Executing action for state Peds_Green" << std::endl;
    // User-defined action code:
    flash_count = 0; output("car_light", "RED"); output("ped_light", "GREEN");
}
...
// Function names include a unique index generated during JSON serialization.
    ...
    // Guard function for transition #3 (Source: Peds_Flash_Off, Target: Peds_Flash_On)
    bool check_guard_3() {
    try {
         // User-defined guard condition code:
         // Uses original variable names and calls valueof("input_name") for inputs.
        return (flash_count < max_flashes);
    ...
    // Guard function for transition #4 (Source: Peds_Flash_On, Target: Peds_Flash_Off)
    bool check_guard_4() {
    try {
         // User-defined guard condition code:
         // Uses original variable names and calls valueof("input_name") for inputs.
        ...
        // Basic error handling for exceptions during guard evaluation.
        std::cerr << "[ERROR] Exception in guard_4: " << e.what() << std::endl;
        return false;
    } catch (...) {
        std::cerr << "[ERROR] Unknown exception in guard_4" << std::endl;
        return false;
    }
}
    ...
    // Guard function for transition #5 (Source: Peds_Flash_Off, Target: Cars_Green)
    bool check_guard_5() {
    try {
         // User-defined guard condition code:
         // Uses original variable names and calls valueof("input_name") for inputs.
        ...
        // Basic error handling for exceptions during guard evaluation.
        std::cerr << "[ERROR] Exception in guard_5: " << e.what() << std::endl;
        return false;
    } catch (...) {
        std::cerr << "[ERROR] Unknown exception in guard_5" << std::endl;
        return false;
    }
}
    ...
    // Guard function for transition #6 (Source: Peds_Flash_On, Target: Cars_Green)
    bool check_guard_6() {
    try {
         // User-defined guard condition code:
         // Uses original variable names and calls valueof("input_name") for inputs.
        ...
        // Basic error handling for exceptions during guard evaluation.
        std::cerr << "[ERROR] Exception in guard_6: " << e.what() << std::endl;
        return false;
    } catch (...) {
        std::cerr << "[ERROR] Unknown exception in guard_6" << std::endl;
        return false;
    }
}
...
void executeCurrentStateAction() {
    ...
    switch (currentState) {
        ...
        case State::STATE_CARS_GREEN: // Case for state: Cars_Green
            action_CARS_GREEN();      // Call generated action function
            break;
        
        case State::STATE_CARS_YELLOW: // Case for state: Cars_Yellow
            action_CARS_YELLOW();      // Call generated action function
            break;
        
        case State::STATE_PEDS_FLASH_OFF: // Case for state: Peds_Flash_Off
            action_PEDS_FLASH_OFF();      // Call generated action function
            break;
        
        case State::STATE_PEDS_FLASH_ON: // Case for state: Peds_Flash_On
            action_PEDS_FLASH_ON();      // Call generated action function
            break;
        
        case State::STATE_PEDS_GREEN: // Case for state: Peds_Green
            action_PEDS_GREEN();      // Call generated action function
            break;
    ...
    { // Scope for temporary stringstream
        std::stringstream ss;
        ss << flash_count; // Convert variable value to string
        engine.sendVarUpdate("flash_count", ss.str()); // Send update
    } 
    
    { // Scope for temporary stringstream
        std::stringstream ss;
        ss << max_flashes; // Convert variable value to string
        engine.sendVarUpdate("max_flashes", ss.str()); // Send update
    } 
    
    { // Scope for temporary stringstream
        std::stringstream ss;
        ss << ped_walk_time; // Convert variable value to string
        engine.sendVarUpdate("ped_walk_time", ss.str()); // Send update
    } 
    ...
    do {
        ...
        if (!event) {
            switch(currentState) {
                ...
                // Check transitions originating from state: Cars_Green
                case State::STATE_CARS_GREEN: {
                    bool guard_ok; // Variable to store guard evaluation 
                     ...
                     end_switch_immediate_STATE_CARS_GREEN:;
                    break;
                    }
                
                // Check transitions originating from state: Cars_Yellow
                case State::STATE_CARS_YELLOW: {
                    bool guard_ok; // Variable to store guard evaluation 
                            ...
                            if (guard_ok) {
                                // Check if it's an immediate or delayed transition.
                                 // Delayed transition.
                                     ...
                                     if (delay_ms >= 0) {
                                         engine.scheduleTimer(delay_ms, stateEnumToName[State::STATE_PEDS_GREEN]); // <<< Použi enum_id cieľa
                                     }
                     ...
                     end_switch_immediate_STATE_CARS_YELLOW:;
                    break;
                    }
                
                // Check transitions originating from state: Peds_Flash_Off
                case State::STATE_PEDS_FLASH_OFF: {
                    bool guard_ok; // Variable to store guard evaluation 
                            ...
                            if (guard_ok) {
                                // Check if it's an immediate or delayed transition.
                                 // Delayed transition.
                                     ...
                                     if (delay_ms >= 0) {
                                         engine.scheduleTimer(delay_ms, stateEnumToName[State::STATE_PEDS_FLASH_ON]); // <<< Použi enum_id cieľa
                                     }
                            ...
                            guard_ok = true; // Assume guard is true unless 
                                ...
                                // Evaluate the guard condition if it exists
                                guard_ok = check_guard_5();
                            
                            if (guard_ok) {
                                // Check if it's an immediate or delayed transition.
                                 // Immediate transition (no delay number, no delay variable).
                                     next_state_candidate = State::STATE_CARS_GREEN; // Set target state.
                                     immediate_transition_found_in_cycle = true;
                                     goto end_switch_immediate_STATE_PEDS_FLASH_OFF;
                     ...
                     end_switch_immediate_STATE_PEDS_FLASH_OFF:;
                    break;
                    }
                
                // Check transitions originating from state: Peds_Flash_On
                case State::STATE_PEDS_FLASH_ON: {
                    bool guard_ok; // Variable to store guard evaluation 
                            ...
                            if (guard_ok) {
                                // Check if it's an immediate or delayed transition.
                                 // Delayed transition.
                                     ...
                                     if (delay_ms >= 0) {
                                         engine.scheduleTimer(delay_ms, stateEnumToName[State::STATE_PEDS_FLASH_OFF]); // <<< Použi enum_id cieľa
                                     }
                            ...
                            if (guard_ok) {
                                // Check if it's an immediate or delayed transition.
                                 // Immediate transition (no delay number, no delay variable).
                                     next_state_candidate = State::STATE_CARS_GREEN; // Set target state.
                                     immediate_transition_found_in_cycle = true;
                                     goto end_switch_immediate_STATE_PEDS_FLASH_ON;
                                
                            }
                        
                     
                     end_switch_immediate_STATE_PEDS_FLASH_ON:;
                    break;
                    }
                
                // Check transitions originating from state: Peds_Green
                case State::STATE_PEDS_GREEN: {
                    bool guard_ok; // Variable to store guard evaluation 
                                ...
                                // Check if it's an immediate or delayed transition.
                                 // Delayed transition.
                                     ...
                                     // Determine the delay value.
                                            ...
                                            try { delay_ms = static_cast<long long>(ped_walk_time); }
                                            catch (...) { delay_ms = -1; std::cerr << "[ERROR] Delay variable 'ped_walk_time' invalid!" << std::endl; }
                     ...
                     end_switch_immediate_STATE_PEDS_GREEN:;
                    break;
                    }
        ...
        // This phase runs only if an external event *is* being processed and no immediate transition was taken in Phase 1.
            ...
            switch(currentState){
                 ...
                 // Check transitions from state: Cars_Green
                 case State::STATE_CARS_GREEN:{
                    bool guard_ok;
                     
                            // Consider transition #0: Cars_Green -> Cars_Yellow
                            
                             if (eventName == "ped_button") {
                                guard_ok = true; // Assume guard ok unless 
                                
                                if(guard_ok) {
                                    // Check if immediate or delayed.
                                     // Immediate event transition.
                                        next_state_event_candidate = State::STATE_CARS_YELLOW; // Set target state.
                                        event_transition_found = true; // Mark event transition found.
                                        goto end_switch_event_STATE_CARS_GREEN; // Use goto to exit the inner loop and switch for this state.
                                    
                                }
                             }
                        
                     
                            // Consider transition #1: Cars_Yellow -> Peds_Green
                            
                     
                            // Consider transition #2: Peds_Green -> Peds_Flash_Off
                            
                     
                            // Consider transition #3: Peds_Flash_Off -> Peds_Flash_On
                            
                     
                            // Consider transition #4: Peds_Flash_On -> Peds_Flash_Off
                            
                     
                            // Consider transition #5: Peds_Flash_Off -> Cars_Green
                            
                     
                            // Consider transition #6: Peds_Flash_On -> Cars_Green
                            
                     
                     end_switch_event_STATE_CARS_GREEN:;
                    break;
                 }
                 
                 // Check transitions from state: Cars_Yellow
                 case State::STATE_CARS_YELLOW:{
                    bool guard_ok;
                     ...
                     end_switch_event_STATE_CARS_YELLOW:;
                    break;
                 }
                 
                 // Check transitions from state: Peds_Flash_Off
                 case State::STATE_PEDS_FLASH_OFF:{
                    bool guard_ok;
                     ...
                     end_switch_event_STATE_PEDS_FLASH_OFF:;
                    break;
                 }
                 
                 // Check transitions from state: Peds_Flash_On
                 case State::STATE_PEDS_FLASH_ON:{
                    bool guard_ok;
                     ...
                     end_switch_event_STATE_PEDS_FLASH_ON:;
                    break;
                 }
                 
                 // Check transitions from state: Peds_Green
                 case State::STATE_PEDS_GREEN:{
                    bool guard_ok;
                     ...
                     end_switch_event_STATE_PEDS_GREEN:;
                    break;
                 }
...
void handleStatusRequestCallback() {
    ...
    
    {
        std::stringstream ss;
        ss << flash_count;
        engine.sendVarUpdate("flash_count", ss.str());
    }
    
    {
        std::stringstream ss;
        ss << max_flashes;
        engine.sendVarUpdate("max_flashes", ss.str());
    }
    
    {
        std::stringstream ss;
        ss << ped_walk_time;
        engine.sendVarUpdate("ped_walk_time", ss.str());
    }
...
int main(int argc, char *argv[]) {
    ...
    stateNameToEnum["Cars_Green"] = State::STATE_CARS_GREEN; // Map name to enum
    stateEnumToName[State::STATE_CARS_GREEN] = "Cars_Green"; // Map enum to name
    
    stateNameToEnum["Cars_Yellow"] = State::STATE_CARS_YELLOW; // Map name to enum
    stateEnumToName[State::STATE_CARS_YELLOW] = "Cars_Yellow"; // Map enum to name
    
    stateNameToEnum["Peds_Flash_Off"] = State::STATE_PEDS_FLASH_OFF; // Map name to enum
    stateEnumToName[State::STATE_PEDS_FLASH_OFF] = "Peds_Flash_Off"; // Map enum to name
    
    stateNameToEnum["Peds_Flash_On"] = State::STATE_PEDS_FLASH_ON; // Map name to enum
    stateEnumToName[State::STATE_PEDS_FLASH_ON] = "Peds_Flash_On"; // Map enum to name
    
    stateNameToEnum["Peds_Green"] = State::STATE_PEDS_GREEN; // Map name to enum
    stateEnumToName[State::STATE_PEDS_GREEN] = "Peds_Green"; // Map enum to name
    

    // --- Set Initial State ---
    // Set the currentState variable based on the initial state specified in the JSON.
    currentState = State::STATE_CARS_GREEN;
    // Basic validation: Check if the generated initial state enum is valid.
    if (currentState == State::STATE_NULL && "Cars_Green" != "") {
         std::cerr << "[ERROR] Initial state 'Cars_Green' issue!" << std::endl; return 1;
    } else if ("Cars_Green" == "") {
